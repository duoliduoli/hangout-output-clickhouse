...
public class TabSeparated implements FormatParse {
    ...
    public void prepare() {
        ...
        if (!this.config.containsKey("database")) {
            log.error("The parameter <database> must be included in config");
            System.exit(1);
        }
        this.database = (String) this.config.get("database");

        if (!this.config.containsKey("table")) {
            log.error("The parameter <table> must be included in config");
            System.exit(1);
        }
        ...
        if(this.config.containsKey("username") && this.config.containsKey("password")) {
            ...
            log.warn("The parameters of <username> and <password> must be included in config at same time");
        } else {
        ...
        try {
            ...
            log.error("Cannot connection to database");
            log.error(e);
            System.exit(1);
        }
        ...
        try {
            ...
            log.error("The input table is invalid");
            System.exit(1);
        }
        ...
        if (this.config.containsKey("fields")) {
            ...
            log.error("The parameter <fields> must be included in config");
            System.exit(1);
        }

        for (String field: fields) {
            if (!this.schema.containsKey(ClickhouseUtils.realField(field))) {
                String msg = String.format("The table [%s] doesn't contain field '%s'", this.table, field);
                log.error(msg);
                System.exit(1);
            }
            try {
